...
get_project_type() {
    ...
    if [ -z "$PROJECT_TYPE" ] || [ "$PROJECT_TYPE" = "unknown" ]; then
        # 检测 HTML/CSS 项目（博客/静态网站）- 优先检测
        if [ -f "$PROJECT_ROOT/index.html" ]; then
            # HTML 项目都是静态网站，使用 static_site 类型
            PROJECT_TYPE="static_site"
        # 检测 Rails 项目
        elif [ -f "$PROJECT_ROOT/Rakefile" ] && [ -d "$PROJECT_ROOT/app" ]; then
            PROJECT_TYPE="rails"
        # 检测 Python 项目
        elif [ -f "$PROJECT_ROOT/app.py" ] || [ -f "$PROJECT_ROOT/main.py" ]; then
            PROJECT_TYPE="api"
        # 检测 Node.js 项目（有 package.json 且有 src 目录）
